  ...
  return (
    ...
    >
      ...
      <figure className="h-48 overflow-hidden relative rounded-t-xl">
        ...
        />
        <div className={`absolute inset-0 bg-gradient-to-t transition-colors duration-500 ${isHovered ? 'from-black/50 to-black/10' : 'from-black/70 to-black/20'}`}></div>
      ...
      <div className="card-body p-6 bg-opacity-90 backdrop-blur-lg relative z-10">
        <h2 className={`card-title text-transparent bg-clip-text bg-gradient-to-r from-purple-400 to-pink-400 text-2xl font-bold mb-3 transform transition-transform duration-300 ${isHovered ? 'translate-x-2' : 'translate-x-0'}`}>
          {title}
        </h2>
        <p className={`text-gray-300 text-sm transform transition-transform duration-300 delay-75 ${isHovered ? 'translate-x-2' : 'translate-x-0'}`}>
          {description}
        </p>

        {/* Action buttons */}
        <div className={`card-actions justify-between mt-6 transform transition-transform duration-300 delay-100 ${isHovered ? 'translate-y-[-4px]' : 'translate-y-0'}`}>
          <a
            href={live}
            target="_blank"
            ...
            onClick={(e) => {
              ...
              circle.classList.add('absolute', 'bg-white', 'rounded-full', 'opacity-30', 'scale-0');
          ...
          <a
            ...
            onClick={(e) => {
              ...
              circle.style.left = `${e.clientX - button.offsetLeft - diameter / 2}px`;
              circle.style.top = `${e.clientY - button.offsetTop - diameter / 2}px`;
              circle.classList.add('absolute', 'bg-purple-500', 'rounded-full', 'opacity-30', 'scale-0');
              circle.style.transform = 'scale(0)';
              circle.style.animation = 'ripple 600ms linear';
